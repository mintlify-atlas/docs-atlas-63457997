...
The stats bar includes a `DotsIcon` button that toggles `isMore` state. When expanded, three additional action buttons appear:
...
{isMore && (
  <>
    <Button startContent={<ShareIcon className='size-5' />}>
      Compartir
    </Button>
    <Button startContent={<EditIcon className='size-5' />}>
      Editar
    </Button>
    <Button color='destructive' startContent={<TrashIcon className='size-5' />}>
      Eliminar
    </Button>
  </>
)}
