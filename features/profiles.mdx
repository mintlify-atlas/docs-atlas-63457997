...
All three update fields (`username`, `description`, `avatar`) are optional — only fields whose values have changed need to be sent. The mutation returns the full updated user object, which Apollo Client uses to update the cache and re-render `InfoProfile` automatically.
...
{isFollowing ? (
  <Button variant='solid' color='primary' onClick={handleFollowing}>
    Siguiendo
  </Button>
) : (
  <Button variant='outline' color='primary' onClick={handleFollowing}>
    Seguir
  </Button>
)}
```
...
A single **Cuenta privada** toggle (`SwitchButton`) backed by the `private` boolean on the `Setting` model. When enabled, the user's posts and profile are not visible to users who do not follow them.
...
|---|---|---|---|
| Calificaciones (desktop) | `n_ratings` | Boolean | `true` |
| Comentarios (desktop) | `n_comments` | Boolean | `true` |
| Nuevos seguidores (desktop) | `n_followers` | Boolean | `true` |
| Componentes populares (desktop) | `n_populates` | Boolean | `true` |
| Calificaciones (email) | `n_email_ratings` | Boolean | `true` |
| Comentarios (email) | `n_email_comments` | Boolean | `true` |
| Nuevos seguidores (email) | `n_email_followers` | Boolean | `true` |
