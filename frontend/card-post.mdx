...
The card header contains three elements arranged in a horizontal row:

- **Author avatar** — rendered with `UserIcon` (size `size-5`).
- **Username link** — wrapped in a `<Link>` pointing to `/home/profile/:id`. Clicking navigates to the author's profile page.
- **Follow/unfollow toggle** — a `Button` whose `variant` and label switch based on local `isFollowing` state (see [Follow Toggle](#follow--unfollow-toggle) below).
- **Timestamp** — a muted `<span>` showing relative time (e.g. `18hrs`), styled with `opacity-75`.
...
The card footer is separated from the body by a top border (`border-t border-seagreen-950/40 dark:border-white/40`) and contains four items:
